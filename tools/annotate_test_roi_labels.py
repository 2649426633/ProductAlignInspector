...
def main() -> None:
    ...
    p.add_argument("--max-width", type=int, default=1700)
    ...
    def load_current() -> None:
        nonlocal current_aligned, current_scale, current_selected, current_key, current_error
    ...
    def render() -> np.ndarray:
        if current_aligned is None:
            ...
            cv2.putText(canvas, f"ALIGN FAILED: {current_error}", (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(canvas, "N next | B previous | Q quit", (20, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
            return canvas

        h, w = current_aligned.shape[:2]
        dw, dh = max(1, int(round(w * current_scale))), max(1, int(round(h * current_scale)))
        canvas = cv2.resize(current_aligned, (dw, dh), interpolation=cv2.INTER_AREA)
        thickness = 2
        for region in regions:
            ...
            cv2.putText(canvas, label, (x, max(18, y - 4)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, color, 1, cv2.LINE_AA)

        bar_h = 58
        canvas = cv2.copyMakeBorder(canvas, bar_h, 0, 0, 0, cv2.BORDER_CONSTANT, value=(20, 20, 20))
